...
if __name__ == "__main__":
    asyncio.run(main())        "TILKY JSON, bez poyasnen!"
    )
    try:
        async with httpx.AsyncClient() as client:
            r = await client.post(
...
async def button_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("done_"):
        ...
        if task:
            assignee = (task[3] or "").replace("@","").lower()
            current = username.lower() if username else user.lower()
            if assignee and assignee != current and assignee != user.lower():
                await query.answer("Тільки " + task[3] + " може закрити це завдання!", show_alert=True)
                return
        await query.answer()
        ok = complete_task(task_id, query.message.chat.id)
        if ok:
